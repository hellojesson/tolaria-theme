...
The downstream theme-extension layer in [ADR-0178](adr/0178-pluggable-theme-extension-overlay.md) is a bounded color-token overlay, not a return to arbitrary vault-authored CSS.
...
5. **Extension package contract** (`src/features/theme-extensions/themePackage.ts`): Validates versioned JSON packages, both resolved-mode variants, a semantic token allowlist, and hex-only colors
6. **Extension runtime** (`themeRuntime.ts` + `useThemeExtensionRuntime.ts`): Applies namespaced localStorage selection after the official mode, clears stale overrides on fallback, and follows resolved System changes across windows
...
Theme-extension selection and imported JSON packages deliberately stay outside `Settings` under `tolaria.theme-extension.*.v1` localStorage keys. This keeps the downstream feature from widening the upstream settings schema or touching vault data.
