...
Theme-extension selection, imported JSON packages, and editor-font preferences deliberately stay outside `Settings` under `tolaria.theme-extension.*.v1` localStorage keys. This keeps the downstream feature from widening the upstream settings schema or touching vault data. Font preferences contain only a preset id or validated local family name; theme files cannot carry font assets or remote font locations.
